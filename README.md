...
​	本小程序用于西安电子科技大学健康卡（疫情通）的自动提交。小程序的理念是在不妨碍疫情防控的前提下，为大部分处于安全区的同学提供便捷的自动提交服务。
...
+ 在云开发控制台数据库页选中集合`data`，点击数据权限，选中`所有用户可读，仅创建者可读写`。
+ 在微信开发者工具中间资源管理器处，找到`cloudfunctions`文件夹，可以看到里面一共有5个文件夹，分别是五个云函数。
...
+ 在每一个云函数的文件夹上面右击，点击`上传并部署：云端安装依赖（不上传node_modules）`，将五个云函数均部署在云端。
